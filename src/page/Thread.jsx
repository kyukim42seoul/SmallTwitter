...
const Thread = (props) => {
  ...
  const getThreadInfos = () => {
    ...
    axios
      .get("http://localhost:3232/threads", {
  ...
  return (
    <StyledThread ref={componentRef} {...props}>
      <StyledMainNavigation>
        <StyledLink
          onClick={() => {
            setCurrentTab(Tab.me);
          }}
          className={isCurrentTab(Tab.me) ? "active" : ""}
        >
          For you
        </StyledLink>
        <StyledLink
          onClick={() => {
            setCurrentTab(Tab.follow);
          }}
          className={isCurrentTab(Tab.follow) ? "active" : ""}
        >
          Following
        </StyledLink>
      </StyledMainNavigation>
      ...
      {threadInfos.map((threadInfo) => (
        <ThreadCard key={uuidv4()} threadInfo={threadInfo} />
      ))}
      <ThreadCard />
      <ThreadCard />
      <ThreadCard />
